...
Bolt::Bolt(const BoltParameters &p) : params(p) {
  ...
  // Place head just above the shank. Use overlap to ensure a clean boolean fuse.
  ...
  // Select largest volume solid (the actual body; others are artifacts)
  ...
  // ── STAGE 1: Underhead fillet (independent) ────────────────────────────
  if (params.head.underheadFilletRadius > 0) {
    try {
      ...
      // Fillet might fail if radius is too large for geometry; keep original.
    }
  }

  // ── STAGE 2: Global edge fillet (independent; was nested inside stage 1) ─
  ...
  // Always assign body — previously this was trapped inside the underhead-if
  // block, causing an uninitialized TopoDS_Solid when fillet was 0.
  body = selected;
}
...
TopoDS_Solid Bolt::Shank() {
  ...
  threadedPart = Cut(threadedPart,
                     BRepBuilderAPI_Transform(trimMask, trimTrans).Shape());
...
TopoDS_Solid Bolt::Head() {
  ...
  if (params.head.type == HeadType::HEX) {
    ...
    socketOffset.SetTranslation(
        gp_Vec(0.0, 0.0, k - params.head.socketDepth));
